...
public abstract class WebUtils {
	...
	public static String getParameter(
		...
		Map map = getURLParameter(request.getQueryString());
	...
	private static Map getURLParameter(String queryString){
		Map returnValue = new HashMap();
		if(queryString!=null && !queryString.equals("")){
			String[] urls = org.apache.commons.lang.StringUtils.split(queryString, "&");
			for (int i = 0; i < urls.length; i++) {
				String[] arr = org.apache.commons.lang.StringUtils.split(urls[i], "=");
				String pname = arr[0];
				String value = null;
				if (arr.length >= 2) {
					String pvalue = arr[1];
					pvalue = java.net.URLDecoder.decode(pvalue);
					value = pvalue;
				} else {
					value = "";
				}
				returnValue.put(pname, value);
			}
		}
		return returnValue;
	}
	 ...
	 * @param timeout
	 */
	public static void saveCookieValue(HttpServletResponse response,String name,String value,int timeout){
		...
		if(timeout<=0){
			if(timeout!=-1) timeout = 60 * 60 * 24 * 365;
		}
	...
	public static String getCookieValue(HttpServletRequest request, String name){
		return getCookieValue(request.getCookies(),name,null);
	}
		...
		if(cookies!=null){
			for(int i=0; i<cookies.length; i++) {
				...
				if (cookiename.equals(cookie.getName())) {
					...
					try {
						returnValue = java.net.URLDecoder.decode(returnValue,"UTF-8");
					} catch (UnsupportedEncodingException e) {
						returnValue = null;
					}
					break;
	...
	/**
	 * 得到真实的IP地址。在通过了Apache，Squid等反向代理软件就不能用request.getRemoteAddr
	 * 获取到客户端的真实IP地址。
	 * @param request
	 * @return
	 */
	...
	/**
	 * 得到不带Http的来源页，用于得到本站内部的来源页。
	 * @param request
	 * @return
	 */
	public static String getReferer(HttpServletRequest request){
		String lastURL = null;
		String http = request.getScheme().toLowerCase() + "://" + request.getServerName().toLowerCase();
		if(request.getServerPort()!=80){
			http = http + ":" + request.getServerPort();
		}
		lastURL = getHttpReferer(request);
		if(lastURL != null && !"".equals(lastURL)){
			if(lastURL.startsWith(http)){
				int index = lastURL.indexOf("/", http.length());
				if(index>0){
					lastURL = lastURL.substring(index);
				} else {
					lastURL = null;
				}
			} else {
