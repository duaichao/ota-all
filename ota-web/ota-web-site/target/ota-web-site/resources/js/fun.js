...
//					console.log(d1+'**d1');
					...
					var nd = d1.getFullYear();
//					console.log(nd+'**nd-d1.getFullYear()');
					var nd1 = d1.getFullYear();
					var price_html = '';
					actual_price = '';
					
					if(d1.getMonth()+1 < 10){
						nd += '0'+(new Number(d1.getMonth())+1);
						nd1 += '-0'+(d1.getMonth(d1.getMonth())+1);
					}else{
						nd += ''+(d1.getMonth()+1);
						nd1 += '-'+(d1.getMonth()+1);
					}
					...
					for(var l = 0; l < tcalendar.length; l++){
						if(tcalendar[l].RQ == nd){
							actual_price = tcalendar[l].ACTUAL_PRICE;
							price_html = '<em class="nl">￥'+tcalendar[l].ACTUAL_PRICE+'</em>';		
							break;
						}
					}
					
					if (i3 < dmin || i3 > dmax) {
						tmp += '><p><em>' + i2 + '</em><br>'+price_html+'</td>';
					} else {
						tmp += ' week="' + (k - 1) + '" price="'+actual_price+'"  id="' + nd1 + '" title="' + ca.getl(d1,false) + ' ' + ca.getst(d1) + ' ' + ca.getsf(d1) + ' ' + ca.getlf(d1) +'"><p><em>' + i2 + '</em><br>'+price_html + (function (t){if($.isFunction(showFu)){return showFu(t);}else{return ""}}(new Date(y,m-1,i2))) +'</p></td>';
