 ...
 *
 */
Ext.define('Weidian.view.ux.calendar.View', {
                    ...
                    '<table class="{[this.me.getViewMode().toLowerCase()]}">',
                            ...
                            '<tr>',
                                '<tpl for="this.getDaysArray(values)">',
                                    '<th class="{[this.getHeaderClass(xindex)]}">',
                                        '<tpl if="xindex === 4">',
                                            '<span>{[Ext.Date.format(this.me.currentDate, "Y")]}/{[Ext.Date.format(this.me.currentDate, "m")]}</span>',
                                        '</tpl>',
                                        '{[util.getShortDayName(values.date)]}',
                                    '</th>',
                                '</tpl>',
                            '</tr>',
                        '</thead>',
                        '<tbody>',
                            ...
                            '<tpl for=".">',
                                ...
                                '<td class="time-block {[this.getClasses(values)]}" dateno="{date:this.formatDate()}" datetime="{[this.me.getDateAttribute(values.date)]}">',
	...
	constructor: function(config){
		...
		var store = Ext.create('Ext.data.Store', {
	        model: Ext.create('Weidian.model.ux.calendar.Model')
	    });
	 ...
	 */
	initModel: function(){
		if(!Ext.ClassManager.isCreated('TouchCalendarViewModel')) // TODO: Throws an error in opening Ext.ux.TouchCalendar.html example?? 
		{
	...
	applyViewMode: function(viewMode){
        ...
        var viewModeFns = Weidian.view.ux.calendar.View[viewMode.toUpperCase()];
	...
	populateStore: function(){
		...
		for(var i = 0; i < totalDays; i++){
            ...
            record = Ext.create('Weidian.model.ux.calendar.Model', {
    ...
    onTimeSlotTap: function(e){
	    if(!e.getTarget('.' + this.getUnselectableCls())){ // don't do selection if the cell has 'unselectable' class
		    var target = Ext.fly(e.getTarget('td'));
		    if(!target.hasCls('able')){
		    	return;
		    }
	...
	statics: {
		...
		MONTH: {
				...
				getStartDate: function(date){
					return Ext.bind(Weidian.view.ux.calendar.View.WEEK.getStartDate, this)(new Date(date.getFullYear(), date.getMonth(), 1));
				},
