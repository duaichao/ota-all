 ...
 * 
 * This extension wraps the Weidian.view.ux.calendar.View in a Ext.Carousel component and allows the calendar to respond to swipe
 * gestures to switch the displayed period. It works by creating 3 Weidian.view.ux.calendar.Views and dynamically creating/removing
 ...
 * 
 */
Ext.define('Weidian.view.ux.calendar.Main',{
	extend: 'Ext.carousel.Carousel',

	xtype: 'ExpandCalendar',
	...
	*/
	...
	initViews: function(){
		...
		items.push(
		    Ext.create('Weidian.view.ux.calendar.View', Ext.applyIf({
		        currentDate: viewValue
		...
		// active view
		items.push(
			Ext.create('Weidian.view.ux.calendar.View', Weidian.view.ux.calendar.View(this.getViewConfig(origCurrentDate)))
		);

		// second out of view (i.e. third)
		viewValue = this.getViewDate(origCurrentDate, 1);
		items.push(
			Ext.create('Weidian.view.ux.calendar.View', Weidian.view.ux.calendar.View(Ext.applyIf({
		        currentDate: viewValue
		    }, this.getViewConfig(viewValue))))
		);
	...
	*/
	...
	onActiveItemChange: function(container, newCard, oldCard){
		if(!container.up())return;
		if (this.getEnableSwipeNavigate()) {
			this.syncHeight(newCard);
			...
			if (direction === 'forward') {
				var newCalendar = new Weidian.view.ux.calendar.View(this.getViewConfig(this.getViewDate(newCard.currentDate, 1)));
	...
	syncHeight:function(view){
		var view = view || this.view,
			len = view.getData().length;
		if(len/7>5){
			this.setHeight(300);
		}else{
			this.setHeight(255);
