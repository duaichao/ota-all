Ext.define('Ext.ux.swfupload.SwfPanel',{
	extend : 'Ext.grid.Panel',
	alias : 'widget.swfpanel',
	width : 700,
	border:true,
	height : 300,
    plugins: [
    ...
    store : Ext.create('Ext.data.JsonStore',{
    	autoLoad : false,
    	fields : ['id','title','type','size','percent','status','href']
    }),
	addFileBtnText : '<i class="iconfont" style="top:1px;">&#xe631;</i> 添加附件',//'添加附件',
	uploadBtnText : '<i class="iconfont" style="top:1px;">&#xe688;</i> 开始上传',//'上传',
	removeBtnText : '<i class="iconfont f18" style="top:1px;">&#xe7fc;</i> 移除全部',//'移除所有',
	cancelBtnText : '<i class="iconfont f18" style="top:1px;">&#xe7fc;</i> 取消',//'取消',
	debug : false,
	file_size_limit : 10,//MB
	file_types : '*.jpg;*.png;*.gif;*.doc;*.docx',
	file_types_description : 'All Files',
	file_upload_limit : 50,
	file_queue_limit : 0,
	post_params : {},
	upload_url : 'test.do',
	flash_url : cfg.getExtPath()+"/src/ux/swfupload/swfupload.swf",
	flash9_url : cfg.getExtPath()+"/src/ux/swfupload/swfupload_fp9.swf",
	initComponent : function(){	
		this.columns = [
            {xtype: 'rownumberer'},
    		{text: '标题', flex:1,dataIndex: 'title',editor: {allowBlank:false,xtype: 'textfield'}},
    		{text: '链接', width: 200,dataIndex: 'href',editor: {allowBlank:false,xtype: 'textfield'}},
            this.getTypeCol()||{text: '类型', width: 70,dataIndex: 'type'},
    		...
    		}},
            {
                ...
                width:50,
                items: [{
                    icon: cfg.getImgPath()+'/icons/upload-cross-small.png',
                    tooltip: '移除附件',
                    html:'',
	...
	swfupload_preload_handler : function(){
		if (!this.support.loading) {
			util.alert('浏览器Flash Player版本太低,不能使用该上传功能！');
			return false;
		}
	},
	...
	upload_start_handler : function(file){
		...
		var myPostParams = {'title':encodeURIComponent(rec.get('title')),'href':encodeURIComponent(rec.get('href'))};
		if(me.getTypeCol()){
			myPostParams.type=rec.get('type');
		}
		this.setPostParams(myPostParams);
	},
	...
	upload_error_handler : function(file, errorCode, message){console.log(file.filestatus);
		var me = this.settings.custom_settings.scope_handler;		
		var rec = me.store.getById(file.id);
       	rec.set('percent', 0);
		rec.set('status', file.filestatus);
	...
	upload_success_handler : function(file, serverData, responseReceived){console.log(file.filestatus);
		var me = this.settings.custom_settings.scope_handler;		
		var rec = me.store.getById(file.id);
	...
	onUpload : function(){
		if (this.swfupload&&this.store.getCount()>0) {
			var isHref = true,isType=true;
			for(var i=0;i<this.store.getCount();i++){
				var d = this.store.getAt(i).data;
				if(d.href==''||d.href=='http://'){
					isHref = false;
				}
				if(this.getTypeCol()){
					if(d.type==''){
