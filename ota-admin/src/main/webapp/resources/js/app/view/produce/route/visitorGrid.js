Ext.define('app.view.produce.route.visitorGrid', {
	...
	initComponent: function() {
        ...
        this.dockedItems = [{
        	...
        	items:[{
        		...
        		handler:function(){
        			var win = util.createEmptyWindow('输入订单编号','&#xe60b;',260,180,[{
        				xtype:'textfield',
        				name:'orderNo',
        				anchor:'100%',
		   				emptyText:'订单编号'
	...
	calcPrice:function(){
		...
		var totalPanel = this.up('form').down('panel[itemId=rightPanel]').down('panel[itemId=totalPanel]'),
			hiddenPanel = this.up('form').down('panel[itemId=rightPanel]'),
			singleRoomField = Ext.getCmp('singleRoomField'),
		...
		for(var i=0;i<lis.length;i++){
			if(lis[i].dom.id=='singleRoomLi'){
				var wmLabel = lis[i].down('label.wm'),
					thLabel = lis[i].down('label.th');
				wmLabel.setHtml('<b>'+singleRoomField.getValue()+' ×</b>'+util.moneyFormat(routeDetail.RETAIL_SINGLE_ROOM,'f12 disable-color'));
				thLabel.setHtml('<b>'+singleRoomField.getValue()+' ×</b>'+util.moneyFormat(routeDetail.INTER_SINGLE_ROOM,'f12 disable-color'));
				continue;
			};
			...
			wmp.push(li.dom.id+'-'+attrName+'-'+cnt[attrName]*pre);
			thp.push(li.dom.id+'-'+attrName+'-'+cnt[attrName]*thpre);
		...
		if(singleRoomField&&singleRoomField.getValue()>0){
			zj = zj+parseFloat(singleRoomField.getValue())*parseFloat(routeDetail.RETAIL_SINGLE_ROOM);
			thzj = thzj+parseFloat(singleRoomField.getValue())*parseFloat(routeDetail.INTER_SINGLE_ROOM);
		}
		
		
		hiddenPanel.down('hidden#saleAmount').setValue(zj);
		hiddenPanel.down('hidden#interAmount').setValue(thzj);
		hiddenPanel.down('hidden#manCount').setValue(cnt['成人']);
		
		hiddenPanel.down('hidden#wmBasePrice').setValue(Ext.encode(wmp));
		hiddenPanel.down('hidden#thBasePrice').setValue(Ext.encode(thp));
