Ext.define('app.view.stat.sale.parentContainer', {
    extend: 'Ext.Panel',
    config:{
    	layout: {
	    	type:'vbox',
    ...
    initComponent: function(){
    	...
    	var defaultParams = {
    		...
    		baseHtml = [
				...
				'<li class="amount">销售：<span class="money-color f14"><dfn>￥</dfn>{{1}SALE_AMOUNT}</span></li>',
	      ...
	      var store = Ext.create('Ext.data.Store', {
	            ...
	            model:Ext.create('Ext.data.Model',{
	            	fields: ['CNT','SUCCESS_CNT','WEIDU','SALE_AMOUNT','INTER_AMOUNT','TRUE_BUY_AMOUNT','TRUE_SALE_AMOUNT',
	            	         'COMPANY_ID','COMPANY','REFUND_AMOUNT','MARKETING_AMOUNT']
	            }),
	            proxy: {
			         type: 'ajax',
			         noCache:true,
			         url: cfg.getCtx()+'/stat/sale/parent/company',
			         reader: {
			             rootProperty: 'data',
	         		    ...
	         		    '<span>{[this.format(values)]}</span>',
	         		    {
	         		        format: function(o) {
	         		        	if(o.groupField=='WEIDU'){
	         		        		return '<span>'+o.name+' 共'+o.rows.length+'家公司</span>';
	         		        	}else{
				...
				columns:[{
			        ...
			        summaryRenderer: function(v){
		            	...
		            	if(Ext.isObject(v)){
		            		...
		            		str = '<div class="disable-color" style="text-align:right;">共'+v+'家公司，合计:</div>';
		            	}
		            	return str;
		            },
			        dataIndex: 'COMPANY',
